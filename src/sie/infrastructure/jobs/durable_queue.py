...
class DurableJobQueue:
    ...
    async def enqueue(self, task_type: str, payload: dict[str, Any], *, available_at: datetime | None = None, max_attempts: int | None = None) -> str:
        job_id = str(uuid.uuid4())
        now = datetime.now(UTC)
        available = available_at or now
        async with self._session_factory() as session:
            await session.execute(text("""INSERT INTO background_jobs
                    (id, task_type, payload, status, attempts, max_attempts,
                     available_at, created_at)
                    VALUES (:id, :task_type, :payload, 'queued', 0, :max_attempts,
                            :available_at, :created_at)"""), {
                "id": job_id, "task_type": task_type, "payload": payload,
                "max_attempts": max_attempts or self._max_attempts,
                "available_at": available, "created_at": now,
            })
    ...
    async def recover_expired(self) -> int:
        ...
        now = datetime.now(UTC)
        async with self._session_factory() as session:
            result = await session.execute(text("""UPDATE background_jobs
                     ...
                     WHERE status='running' AND leased_until IS NOT NULL
                       AND leased_until < :now"""), {"now": now, "completed_at": now})
            await session.commit()
    ...
    async def claim(self, worker_id: str) -> dict[str, Any] | None:
        ...
        async with self._session_factory() as session:
            ...
            if row is None:
                ...
                return None
            job_id = row["id"]
            updated = await session.execute(text("""UPDATE background_jobs
                ...
                "id": job_id, "worker_id": worker_id, "leased_until": lease,
    ...
    async def complete(self, job_id: str, result: dict[str, Any] | None = None, *, worker_id: str | None = None) -> bool:
        """Complete a job only if this worker still owns it."""
        now = datetime.now(UTC)
        async with self._session_factory() as session:
            predicate = "id=:id AND status='running'"
            params: dict[str, Any] = {"id": job_id}
            if worker_id is not None:
                predicate += " AND worker_id=:worker_id"
                params["worker_id"] = worker_id
            updated = await session.execute(text(f"""UPDATE background_jobs
                       SET status='completed', result=:result, completed_at=:completed_at,
                           leased_until=NULL, worker_id=NULL
                     WHERE {predicate}"""), {**params, "result": result, "completed_at": now})
            await session.commit()
            return updated.rowcount == 1

    async def fail(self, job_id: str, error: str, *, retry: bool = True, worker_id: str | None = None) -> None:
        now = datetime.now(UTC)
        async with self._session_factory() as session:
            row = (await session.execute(text("SELECT attempts, max_attempts FROM background_jobs WHERE id=:id"), {"id": job_id})).mappings().first()
            if row is None:
                raise JobNotFoundError(job_id)
            ...
            params: dict[str, Any] = {"id": job_id}
            if worker_id is not None:
                predicate += " AND status='running' AND worker_id=:worker_id"
                params["worker_id"] = worker_id
            await session.execute(text(f"""UPDATE background_jobs
                       SET status=:status, last_error=:error, leased_until=NULL, worker_id=NULL,
                           completed_at=:completed_at
                     WHERE {predicate}"""), {
                **params, "status": "failed" if terminal else "queued",
                "error": error[:8000], "completed_at": now if terminal else None,
            })
            await session.commit()
    ...
    async def run_worker(self, worker_id: str, *, poll_interval_seconds: float = 1.0, stop_event: asyncio.Event | None = None) -> None:
        ...
        while not stop_event.is_set():
            ...
            if handler is None:
                await self.fail(job["id"], f"No handler registered for task type {job['task_type']!r}", retry=False, worker_id=worker_id)
                continue
            try:
