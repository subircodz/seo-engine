...
def _feature_to_dict(feature: SearchSERPFeature) -> dict[str, Any]:
    return {"feature_type": feature.feature_type.value, "position": feature.position, "title": feature.title, "url": feature.url, "domain": feature.domain, "metadata": list(feature.metadata)}
...
def _feature_from_dict(value: dict[str, Any]) -> SearchSERPFeature:
    return SearchSERPFeature(
        ...
        position=value.get("position"), title=value.get("title"), url=value.get("url"),
...
def _search_to_dict(result: SearchResult) -> dict[str, Any]:
    return {
        "keyword": result.keyword, "search_engine": result.search_engine,
        "country": result.country, "language": result.language, "device": result.device.value,
        "items": [
            {"position": i.position, "title": i.title, "url": i.url, "serp_features": [_feature_to_dict(f) for f in i.serp_features]}
...
def _search_from_dict(value: dict[str, Any]) -> SearchResult:
    ...
    return SearchResult(
        keyword=value["keyword"], search_engine=value["search_engine"], country=value["country"],
        language=value["language"], device=SearchDevice(value["device"]),
        items=tuple(
            SearchResultItem(
                position=i["position"], title=i["title"], url=i["url"],
                serp_features=tuple(_feature_from_dict(f) for f in i.get("serp_features", [])),
...
def _aio_to_dict(value: AIOverviewObservation) -> dict[str, Any]:
    return {
        "keyword": value.keyword, "ai_type": value.ai_type.value, "present": value.present,
        "target_cited": value.target_cited, "target_domain": value.target_domain,
        "citation_count": value.citation_count,
        "citations": [
            {"domain": c.domain, "url": c.url, "position": c.position, "source_type": c.source_type.value, "title": c.title}
            for c in value.citations
        ],
        "competitor_cited_domains": list(value.competitor_cited_domains),
        "observed_at": value.observed_at.isoformat(), "source": value.source,
    }


def _aio_from_dict(value: dict[str, Any]) -> AIOverviewObservation:
    return AIOverviewObservation(
        keyword=value["keyword"], ai_type=AIOverviewType(value["ai_type"]), present=bool(value["present"]),
        target_cited=bool(value.get("target_cited", False)), target_domain=value.get("target_domain", ""),
        citation_count=int(value.get("citation_count", 0)),
        citations=tuple(
            AIOCitation(
                domain=c["domain"], url=c.get("url", ""), position=int(c.get("position", 0)),
                source_type=CitationSource(c.get("source_type", CitationSource.WEB_PAGE.value)), title=c.get("title", ""),
            ) for c in value.get("citations", [])
        ),
        competitor_cited_domains=tuple(value.get("competitor_cited_domains", [])),
        observed_at=datetime.fromisoformat(value["observed_at"]), source=value.get("source", "redis-cache"),
    )


def _geo_to_dict(value: GEOObservation) -> dict[str, Any]:
    return {
        "keyword": value.keyword, "engine_type": value.engine_type.value,
        "target_mentioned": value.target_mentioned, "target_domain": value.target_domain,
        ...
        "competitor_domains": list(value.competitor_domains), "citation_urls": list(value.citation_urls),
        "answer_length": value.answer_length, "observed_at": value.observed_at.isoformat(), "source": value.source,
    }


def _geo_from_dict(value: dict[str, Any]) -> GEOObservation:
    return GEOObservation(
        keyword=value["keyword"], engine_type=GenerativeEngineType(value["engine_type"]),
        target_mentioned=bool(value.get("target_mentioned", False)), target_domain=value.get("target_domain", ""),
        mention_count=int(value.get("mention_count", 0)),
        entity_mentions=tuple(
            EntityMention(text=e["text"], entity_type=EntityType(e.get("entity_type", EntityType.BRAND.value)), is_target=bool(e.get("is_target", False)), domain=e.get("domain", ""))
            for e in value.get("entity_mentions", [])
        ),
        competitor_domains=tuple(value.get("competitor_domains", [])), citation_urls=tuple(value.get("citation_urls", [])),
        answer_length=int(value.get("answer_length", 0)), observed_at=datetime.fromisoformat(value["observed_at"]),
...
class DistributedCacheSearchProvider:
    ...
    def _key(self, query: SearchQuery, *, context: str = "") -> CacheKey:
        return CacheKey.from_query(
            keyword=query.query, country=query.country, language=query.language,
            device=query.device.value, provider=type(self._provider).__name__,
            search_engine=query.search_engine, context=context,
        )
    ...
    async def extract_aio(self, query: SearchQuery, target_domain: str) -> AIOverviewObservation | None:
        ...
        key = self._key(query, context=f"aio:{target_domain.casefold().removeprefix('www.')}")
    ...
    async def query_geo(self, query: SearchQuery, target_domain: str, engine_type: GenerativeEngineType) -> GEOObservation | None:
        ...
        key = self._key(query, context=f"geo:{engine_type.value}:{target_domain.casefold().removeprefix('www.')}")
