...
DEFAULT_GEO_TEMPLATES: dict[GenerativeEngineType, list[GEOPromptTemplate]] = {
    GenerativeEngineType.CHATGPT: [
        GEOPromptTemplate(
            ...
            "accurate answers. When mentioning brands, companies, "
            "or products, be specific and factual.",
    ...
    GenerativeEngineType.PERPLEXITY: [
        GEOPromptTemplate(
            "You are a research assistant that provides well-cited, "
            "factual answers. Always cite your sources and mention "
            "specific brands, companies, and products.",
    ...
    GenerativeEngineType.CLAUDE: [
        GEOPromptTemplate(
            "You are a helpful, accurate assistant. Provide thorough, "
            "nuanced answers. Mention specific brands, companies, "
            "and products when relevant.",
    ...
    GenerativeEngineType.GEMINI: [
        GEOPromptTemplate(
            "You are a helpful assistant that provides comprehensive, "
            "accurate answers. Mention specific brands, companies, "
            "and products when relevant.",
...
class GEOLLMProvider:
    ...
    def _parse_geo_response(
        ...
        for comp_domain in self._competitor_domains:
            matches = list(
                re.finditer(rf"\b{re.escape(comp_domain)}\b", response_text, re.IGNORECASE)
            )
            if matches:
        ...
        for match in re.finditer(
            r"\b[a-z0-9][-a-z0-9]*\.[a-z]{2,}\b", response_text, re.IGNORECASE
        ):
            domain = match.group(0).casefold()
            if (
                domain != target_domain_normalized
                and domain not in self._competitor_domains
                and any(
                    domain.endswith(f".{tld}")
                    for tld in ("com", "org", "net", "io", "co", "ai", "app")
                )
            ):
                competitor_domains.add(domain)
        ...
        return GEOObservation(
            ...
            provenance=EvidenceProvenance(
                ...
                methodology=(
                    "Prompted LLM response; not a live query to the named generative engine."
                ),
            ),
        )
