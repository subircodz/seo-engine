...
class SiteAIOAnalysis:
    ...
    freshness_status: str = (
        "FRESH"  # FRESH, STALE, NEVER_COLLECTED, COLLECTION_FAILED, NOT_ASSESSED
    )
    collected_at: datetime = field(default_factory=lambda: datetime.now(UTC))
...
class SiteAnalysisService:
    ...
    def __init__(
        ...
        if isinstance(search_provider, ProviderRegistry):
            ...
            self._search_provider = (
                search_provider.get_for_aio()
                or search_provider.get_for_geo()
                or search_provider.get_for_rankings()
            )
    ...
    COUNTRY_NAMES: ClassVar[dict[str, str]] = {
        "us": "United States",
        "gb": "United Kingdom",
        "de": "Germany",
        "fr": "France",
    ...
    async def _analyze_country(
        ...
        with contextlib.suppress(Exception):
            aio_analysis, _aio_result = await self._analyze_aio(
        ...
        geo_analysis = None
        with contextlib.suppress(Exception):
            geo_analysis, _geo_result = await self._analyze_geo(
                domain, target_keywords, country_code, device
            )
        ...
        if ranking_snapshot:
            if (
                ...
                action_items.append(f"Register Google Business Profile for {country_name} presence")
            ...
            if ranking_snapshot.visibility_score < 0.2:
                recs.append(
                    SearchRecommendation(
                        ...
                        title=f"Low search visibility in {country_name}"
                            f"({ranking_snapshot.visibility_score:.0%})",
                        description=(
                ...
                action_items.append(f"Implement hreflang tags for {country_name} content")

        if (
            aio_analysis
            and aio_analysis.ai_overviews_present > 0
            and aio_analysis.target_cited_count == 0
        ):
            recs.append(
                SearchRecommendation(
                    ...
                    description=(
                        f"In {country_name}, AI Overviews appear for "
                        f"{aio_analysis.ai_overviews_present} keywords "
            ...
            action_items.append(
                f"Create content targeting "
                f"{country_name}-specific search queries "
                f"with direct answers"
            )
            action_items.append(
        ...
        if (
            geo_analysis
            and geo_analysis.target_mentioned_count == 0
            and geo_analysis.competitor_mentioned_count > 0
        ):
            recs.append(
                SearchRecommendation(
                    ...
                    description=(
                        f"Competitors are mentioned "
                        f"{geo_analysis.competitor_mentioned_count} times "
                        f"in generative engines for {country_name} "
                        f"queries. Build authority."
                    ),
                    confidence=0.8,
                )
            )
            action_items.append(f"Build citations on {country_name}-specific authority domains")
            action_items.append("Create factual, well-structured content that AI engines cite")
    ...
    async def _analyze_all_countries(
        ...
        for cc in target_countries:
            try:
                cr = await self._analyze_country(domain, cc, target_keywords, crawl_pages, device)
                results.append(cr)
    ...
    async def analyze_site(
        ...
        *,
        ...
        content_quality = await self._content_service.generate_quality_report(
            crawl_run_id, content_metrics
        )
                ...
                opt_kwargs.update(
                    {
                        ...
                        "volatile_keyword_count": sum(
                            1 for v in search_intel.volatility_metrics if v.volatility_index > 0.5
                        ),
                    ...
                    opt_kwargs.update(
                        {
                            ...
                            "content_gap_keywords": tuple(c.keyword for c in opp.content_gaps[:10]),
                        }
                    )
            if aio_analysis:
                opt_kwargs.update(
        ...
        return SiteAnalysisResult(
            ...
            search_opportunities=search_intel.opportunity_result
            if search_intel and hasattr(search_intel, "opportunity_result")
            else None,
    ...
    async def _persist_aio_geo_observations(
        ...
        try:
            ...
            if aio_analysis and aio_analysis.query_details:
                ...
                for detail in aio_analysis.query_details:
                    obs = AIOverviewObservation(
                        ...
                        if detail.ai_overview_detected
                        else AIOverviewType.OTHER,
                        ...
                        competitor_cited_domains=tuple(
                            d
                            for d in detail.cited_sources
                            if d.split("/")[2].lower().removeprefix("www.")
                ...
                if aio_observations:
                    await self._repository.save_aio_observations(
                        f"site-aio-{domain}", aio_observations
                    )
            ...
            if geo_analysis and geo_analysis.query_details:
                ...
                if geo_observations:
                    await self._repository.save_geo_observations(
                        f"site-geo-{domain}", geo_observations
                    )
    ...
    def _detect_access_status(self, crawl_pages: list) -> AccessStatus:
        ...
        accessible = (
            first_page.status_code == 200 and not is_challenge and protection_detected is None
        )
    ...
    def _detect_website_type(self, crawl_pages: list) -> WebsiteTypeInfo:
        ...
        if tech_signals:
            ...
            return WebsiteTypeInfo(
                ...
                cms=winner
                if winner in ("WordPress", "Shopify", "Drupal", "Joomla", "Squarespace")
                else None,
        ...
        return WebsiteTypeInfo(
            technology="Unknown",
            confidence=0.0,
            evidence=["Insufficient indicators to determine technology"]
            if not evidence
            else evidence[:5],
            language=language,
            server=server,
        ...
        for i, page in enumerate(crawl_pages[:30]):
            ...
            try:
                ...
                filtered = [
                    w
                    for w in words
                    if w not in STOPWORDS and len(w) >= 3 and not self._is_url_or_domain_token(w)
                ]
                ...
                for j in range(len(filtered) - 2):
                    ...
                    if (
                        filtered[j] not in STOPWORDS
                        and filtered[j + 1] not in STOPWORDS
                        and filtered[j + 2] not in STOPWORDS
                    ):
    ...
    async def _discover_rankings(
        ...
        *,
        ...
        for kw in target_keywords:
            ...
            if kw_obs:
                ...
                detail = KeywordRankingDetail(
                    ...
                    evidence=f"Position #{obs.position} observed in Google SERP for"
                        f"{country.upper()}",
                    status="Verified",
                    confidence="HIGH",
                )
                ...
                detail = KeywordRankingDetail(
                    ...
                    checked_to="Top 100",
                    evidence=f"No ranking found in top 100 results for {country.upper()} Google"
                        f"search",
        ...
        top_kws = sorted(
            [
                {"keyword": o.keyword, "position": o.position, "url": o.target_url}
        ...
        if not_ranking > 0:
            ...
            if missing_kws:
                content_gap = ContentGapDetail(
                    ...
                    recommendation=f"Create comprehensive content targeting:"
                        f"{', '.join(missing_kws[:5])}",
                )
        ...
        authority_gap = AuthorityGapDetail(
            ...
            limitation="No reliable backlink dataset available during this analysis. Therefore, the"
                "report does not make an authority-gap claim based on backlink data.",
            confidence="LOW",
        )
    ...
    def _get_aio_provider(self) -> SearchProvider | None:
        """Get the AIO-capable provider from registry or single provider (legacy single-provider"
            "mode)."""
        if self._provider_registry:
            return self._provider_registry.get_for_aio()
        return (
            self._search_provider if getattr(self._search_provider, "supports_aio", False) else None
        )
    ...
    def _get_geo_provider(self) -> SearchProvider | None:
        ...
        return (
            self._search_provider if getattr(self._search_provider, "supports_geo", False) else None
        )
            ...
            return (
                SiteAIOAnalysis(
                    ...
                    detection_methodology=(
                        "AIO analysis requires a search provider that supports AI Overview"
                            "extraction. "
        ...
        try:
            ...
            for kw in keywords[:20]:
                ...
                for aio_provider in aio_providers:
                    try:
                        ...
                        logger.warning("AIO provider authentication failed for '%s': %s", kw, exc)
                        last_exception = exc
                        ...
                        logger.warning("AIO provider retryable error for '%s': %s", kw, exc)
                        last_exception = exc
                        continue  # Try next provider

                    except SearchProviderError as exc:
                        # Other provider errors (5xx, transport errors) - retryable
                        logger.warning("AIO provider error for '%s': %s", kw, exc)
                        last_exception = exc
                        continue  # Try next provider
                ...
                cited_sources = (
                    [c.url for c in observation.citations] if observation.citations else []
                )
                query_details.append(
                    AIODetailQuery(
                        keyword=kw,
                        ...
                        evidence=(
                            ...
                            + (
                                f". Target cited: {observation.target_cited}"
            ...
            if not aio_observations:
                return (
                    SiteAIOAnalysis(
                        ...
                        detection_methodology="Provider supports AIO but no observations were"
                            "returned",
                    ),
                    None,
                )
            ...
            return (
                SiteAIOAnalysis(
                    ...
                    competitor_cited_count=sum(
                        km.competitor_cited_count for km in aio_result.keyword_metrics
                    ),
                    ...
                    competitor_domains_cited=list(
                        aio_result.dataset_metrics.competitor_cited_domains
                    ),
                    top_opportunities=opportunities[:10],
                    query_details=query_details,
                    detection_methodology="SERP feature analysis via provider AIO extraction with"
                        "fallback",
                ),
                aio_result,
            )
        ...
        try:
            ...
            if not geo_observations:
                return (
                    SiteGEOAnalysis(
                        ...
                        avg_mention_count=0.0,
                        query_details=query_details,
                        detection_methodology="Provider supports GEO but no observations were"
                            "returned",
                    ...
                    opportunities.append(
                        {
                            ...
                            "action": "Build authoritative content and citations for generative"
                                "engines",
                        }
                    )
            ...
            return (
                SiteGEOAnalysis(
                    ...
                    competitor_mentioned_count=sum(
                        km.competitor_mentioned_count for km in geo_result.keyword_metrics
                    ),
                    ...
                    competitor_domains_mentioned=list(
                        geo_result.dataset_metrics.competitor_domain_counts.keys()
                    ),
                    top_opportunities=opportunities[:10],
                    query_details=query_details,
                    detection_methodology=f"Generative engine response analysis via provider GEO"
                        f"query ({engine_type.value})",
                    engines_tested=engines_tested,
                ),
                geo_result,
            )
        ...
        try:
            ...
            if wiki_aligned == 0:
                recommendations.append(
                    "No Wikipedia-aligned entities found - add notable entities with Wikipedia"
                        "links"
                )
            if wikidata_aligned == 0:
                recommendations.append(
                    "No Wikidata-aligned entities - add structured data with sameAs to Wikidata"
                )
            if missing:
                recommendations.append(
                    f"Missing entity types: {', '.join(missing)} - add content covering these types"
                )
        ...
        try:
            ...
            if not page_results:
                return SitePerformanceSummary(
                    pages_analyzed=0,
                    methodology="No HTML pages to analyze",
                )

            dataset_metrics = analyze_dataset_performance("site-analysis", page_results)
            ...
            least_eff = sorted(
                [
                    (r.url, r.metrics.content_efficiency)
                    for r in page_results
                    if r.metrics.content_efficiency > 0
                ],
                key=lambda x: x[1],
            )[:5]
            ...
            if dataset_metrics.avg_content_efficiency < 0.3:
                top_issues.append(
                    f"Low content efficiency ({dataset_metrics.avg_content_efficiency:.0%}) —"
                        f"excessive markup"
                )
            ...
            return SitePerformanceSummary(
                pages_analyzed=0,
                methodology=f"Analysis failed: {e}",
            )
    ...
    def _correct_geo_analysis(self, geo: SiteGEOAnalysis) -> SiteGEOAnalysis:
        ...
        if (
            ...
            return SiteGEOAnalysis(
                ...
                detection_methodology=(
                    "GEO analysis was attempted but the provider returned synthetic/no-op"
                        "observations. "
    ...
    def _build_technical_health(
        ...
        avg_load = sum(p.duration_ms for p in crawl_pages if p.duration_ms) / max(
            len(crawl_pages), 1
        )
    ...
    def _build_technical_metric_details(
        ...
        metrics.append(
            TechnicalMetricDetail(
                ...
                what_failed=f"{total_pages - indexable_count} pages returned non-200 status"
                if crawlability_score < 90
                else "",
                why_it_matters="Pages that return non-200 status codes cannot be crawled or indexed"
                    "by search engines",
                remediation="Fix broken links, implement proper redirects for moved pages, ensure"
                    "server returns 200 for accessible content",
        ...
        metrics.append(
            TechnicalMetricDetail(
                ...
                why_it_matters="HTTPS is a confirmed Google ranking signal. Mixed content degrades"
                    "user trust and security.",
                remediation="Ensure all pages are served over HTTPS. Update any HTTP resources to"
                    "HTTPS.",
        ...
        metrics.append(
            TechnicalMetricDetail(
                ...
                evidence="robots.txt was accessible during crawl"
                + (f" but {robots_issues} pages had missing directives" if robots_issues else ""),
                what_was_checked="robots.txt presence and directives",
                what_failed=f"{robots_issues} pages may have missing or improper robots.txt"
                    f"directives"
                if robots_issues
                else "",
                why_it_matters="Robots.txt controls which pages search engines can crawl. Missing"
                    "or incorrect directives can block important pages.",
                remediation="Create/update robots.txt with proper Allow/Disallow directives for all"
                    "important sections",
        ...
        metrics.append(
            TechnicalMetricDetail(
                ...
                why_it_matters="Sitemaps help search engines discover all pages efficiently,"
                    "especially for large sites.",
                remediation="Ensure sitemap.xml is up to date, includes all important pages, and is"
        ...
        metrics.append(
            TechnicalMetricDetail(
                ...
                why_it_matters="Slow pages increase bounce rate and reduce crawl efficiency. Google"
                    "uses page speed as a ranking signal.",
                remediation="Optimize server response time, compress assets, enable caching, use a"
                    "CDN",
        ...
        metrics.append(
            TechnicalMetricDetail(
                ...
                evidence=f"{audit.total_issues} total issues across {total_pages} pages"
                    f"({issue_density:.1f} issues/page)",
                what_was_checked="SEO audit rule violations per page",
                what_failed=f"{audit.critical_count} critical, {audit.warning_count} warnings,"
                    f"{audit.info_count} info issues"
                if audit.total_issues > 0
                else "",
                why_it_matters="High issue density indicates systemic SEO problems that affect"
                    "overall site health.",
                remediation="Address critical issues first, then systematically fix warnings. Use"
        ...
        metrics.append(
            TechnicalMetricDetail(
                ...
                evidence=f"{indexable_count} of {total_pages} pages are indexable"
                    f"({indexable_pct:.0f}%)",
                ...
                remediation="Ensure all important pages return HTTP 200 and don't have noindex"
                    "directives",
    ...
    def _build_content_breakdown(
        ...
        metrics.append(
            AnalysisMetric(
                ...
                metric_type="binary",
                expected=100.0,
                normalized_score=min(100, title_score),
                weight=12.0,
                raw_value=title_pass,
                raw_unit="pages",
                raw_evidence=f"{title_pass}/{total} pages have non-empty, <=60 char titles",
                normalization_method="Binary per page, then percentage; -5 per duplicate title"
                    "group",
                status="PASS"
                if title_score >= 90
                ...
                why_it_matters="Titles are the primary SERP click-through signal. Missing, empty,"
                    "or duplicate titles reduce CTR and ranking potential.",
                remediation="Write unique, descriptive titles (30-60 chars) for every page.",
                sample_size=total,
                data_coverage=1.0 if total > 0 else 0.0,
        ...
        metrics.append(
            AnalysisMetric(
                ...
                metric_type="binary",
                expected=100.0,
                normalized_score=meta_desc_score,
                weight=12.0,
                raw_value=meta_desc_pass,
                raw_unit="pages",
                raw_evidence=f"{meta_desc_pass}/{total} pages have meta descriptions",
                normalization_method="Binary per page, then percentage",
                status="PASS"
                if meta_desc_score >= 90
                else "NEEDS IMPROVEMENT"
                if meta_desc_score >= 60
                else "POOR",
                evidence=f"{meta_desc_pass} of {total} pages have meta descriptions in 120-160 char"
                    f"range",
                evidence_quality="OBSERVED",
                affected_urls=meta_desc_urls,
                why_it_matters="Meta descriptions are SERP ad copy that directly impact"
                    "click-through rates.",
                remediation="Write unique, compelling 120-160 character descriptions with target"
                    "keywords.",
                sample_size=total,
                data_coverage=1.0 if total > 0 else 0.0,
                coverage_methodology="All crawled pages analyzed",
        ...
        metrics.append(
            AnalysisMetric(
                metric_name="Heading Structure",
                category="content",
                metric_type="binary",
                expected=100.0,
                normalized_score=heading_score,
                weight=10.0,
                raw_value=heading_pass,
                raw_unit="pages",
                ...
                why_it_matters="H1 tags are the strongest on-page signal for page topic. Missing"
                    "H1s make ranking for target keywords harder.",
                remediation="Add a single, descriptive H1 tag to each page that includes the"
        ...
        metrics.append(
            AnalysisMetric(
                metric_name="Content Depth (Word Count)",
                category="content",
                metric_type="threshold",
                expected=100.0,
                normalized_score=depth_score,
                weight=12.0,
                raw_value=depth_score,
                raw_unit="score",
                raw_evidence=f"Avg word-count score: {depth_score:.1f}/100 across {total} pages",
                normalization_method="Per-page: 100 if words >= page-type threshold, else"
                    "(words/threshold)*100; then average",
                status="PASS"
                if depth_score >= 80
                else "NEEDS IMPROVEMENT"
                if depth_score >= 50
                else "POOR",
                evidence=f"Average content depth score: {depth_score:.1f} across {total} pages"
                    f"(page-type-aware thresholds)",
                evidence_quality="CALCULATED",
                affected_urls=depth_urls[:10],
                why_it_matters="Search engines prefer comprehensive content. However, word count is"
                    "a heuristic — low word count alone does not prove poor-quality content. Page"
                        "purpose matters.",
                remediation="Expand thin pages with comprehensive, unique content relevant to the"
                    "page's purpose.",
                limitations="Word count is a heuristic indicator of content depth, not a definitive"
                    "measure of content quality. A page with fewer words may still provide high"
                        "value depending on its purpose.",
                sample_size=total,
                data_coverage=1.0 if total > 0 else 0.0,
        ...
        alignment_score = (
            (sum(alignment_scores) / len(alignment_scores)) if alignment_scores else 50.0
        )
        metrics.append(
            AnalysisMetric(
                metric_name="Semantic Alignment",
                category="content",
                metric_type="composite",
                expected=100.0,
                normalized_score=alignment_score,
                weight=6.0,
                raw_value=alignment_score,
                raw_unit="score",
                raw_evidence=f"Avg heading-keyword overlap: {alignment_score:.1f}%",
                normalization_method="HeadingAnalysis.heading_keyword_coverage * 100, averaged"
                    "across pages",
                status="PASS"
                ...
                evidence=f"Average heading-keyword coverage: {alignment_score:.1f}% across {total}"
                    f"pages",
                evidence_quality="CALCULATED",
                why_it_matters="Headings that align with content keywords signal topical relevance"
                    "to search engines.",
                remediation="Ensure headings contain keywords naturally and reflect the page's"
                    "content structure.",
                sample_size=total,
                data_coverage=1.0 if total > 0 else 0.0,
        ...
        metrics.append(
            AnalysisMetric(
                metric_name="Content Uniqueness",
                category="content",
                metric_type="percentage",
                expected=100.0,
                normalized_score=uniqueness,
                weight=14.0,
                raw_value=dup_count,
                raw_unit="duplicate groups",
                ...
                why_it_matters="Duplicate content causes keyword cannibalization — search engines"
                    "must choose which page to rank, often ranking neither well.",
                remediation="Consolidate duplicate pages, implement canonical tags, use 301"
                    "redirects.",
                sample_size=total,
                data_coverage=1.0 if total > 0 else 0.0,
        ...
        metrics.append(
            AnalysisMetric(
                metric_name="Image Accessibility",
                category="content",
                metric_type="percentage",
                expected=100.0,
                normalized_score=alt_coverage,
                weight=8.0,
                raw_value=images_with_alt,
                raw_unit="images",
                ...
                why_it_matters="Alt text is crucial for accessibility and image search. Missing alt"
                    "text hurts both accessibility and Google Images traffic.",
                remediation="Add descriptive alt text to all images with relevant keywords.",
                sample_size=total_images,
                data_coverage=1.0 if total_images > 0 else 0.0,
        ...
        metrics.append(
            AnalysisMetric(
                metric_name="Internal Linking",
                category="content",
                metric_type="threshold",
                expected=100.0,
                normalized_score=linking_score,
                weight=10.0,
                raw_value=linking_pass,
                raw_unit="pages",
                ...
                evidence=f"{linking_pass} of {total} pages have sufficient internal linking (>=2"
                    f"incoming links)",
                evidence_quality="OBSERVED",
                affected_urls=linking_urls[:10],
                why_it_matters="Internal links distribute PageRank and help users and search"
                    "engines discover content.",
                remediation="Add 3-5 relevant internal links per page using descriptive anchor"
                    "text.",
                sample_size=total,
                data_coverage=1.0 if total > 0 else 0.0,
                coverage_methodology="All crawled pages analyzed",
        ...
        readability_score = (
            (sum(readability_scores) / len(readability_scores)) if readability_scores else 50
        )
        ...
        metrics.append(
            AnalysisMetric(
                ...
                weight=8.0,
                raw_value=readability_score,
                raw_unit="score",
                raw_evidence=f"Avg readability score: {readability_score:.1f}/100",
                normalization_method="Piecewise linear: Flesch 0->0, 30->30, 60->70, 80->90,"
                    "100->100",
                status="PASS"
                ...
                remediation="Use shorter sentences, simpler words, subheadings, bullet points."
        ...
        metrics.append(
            AnalysisMetric(
                ...
                why_it_matters="Content freshness is a ranking factor. Old content without date"
                    "signals appears outdated.",
        ...
        metrics.append(
            AnalysisMetric(
                metric_name="Structured Data",
                category="content",
                metric_type="binary",
                expected=100.0,
                normalized_score=schema_score,
                weight=4.0,
                raw_value=schema_count,
                raw_unit="pages",
                ...
                why_it_matters="Structured data helps search engines understand content and enables"
                    "rich snippets.",
        ...
        overall = (
            sum(m.weight * m.normalized_score for m in metrics) / total_weight
            if total_weight > 0
            else 0
        )
        ...
        avg_evidence = (
            sum(evidence_scores.get(m.evidence_quality, 0) for m in metrics) / len(metrics)
            if metrics
            else 0
        )
        ...
        return CategoryScore(
            ...
            scoring_methodology="Score = sum(weight * normalized_score) / sum(weights) across 10"
                "metrics. Each metric normalized to 0-100.",
            metrics=metrics,
            sample_size=total,
            data_coverage=coverage,
            confidence=confidence,
            confidence_factors=[
                f"{total} pages analyzed",
                f"Evidence quality: avg {avg_evidence:.1f}/3",
            ],
            limitations=[
                "Word count is a heuristic, not a definitive quality measure.",
                "Semantic alignment uses word overlap, not semantic understanding.",
            ],
    ...
    def _build_ranking_breakdown(
        ...
        b4 = (
            max(0, len(snapshot.top_keywords) - snapshot.keywords_in_top_20)
        ...
        metrics: list[AnalysisMetric] = [
            AnalysisMetric(
                metric_name="Top 3 Visibility",
                category="ranking",
                metric_type="percentage",
                expected=100.0,
                normalized_score=(b1 / N * 100) if N > 0 else 0,
                weight=30.0,
                raw_value=b1,
                raw_unit="keywords",
                ...
                evidence=f"{b1} keywords found in top 3 positions for {country.upper()} {device}"
                    f"Google search",
                evidence_quality="OBSERVED",
                sample_size=N,
            ...
            AnalysisMetric(
                metric_name="Positions 4-10",
                category="ranking",
                metric_type="percentage",
                expected=100.0,
                normalized_score=(b2 / N * 100) if N > 0 else 0,
                weight=30.0,
                raw_value=b2,
                raw_unit="keywords",
            ...
            AnalysisMetric(
                metric_name="Positions 11-20",
                category="ranking",
                metric_type="percentage",
                expected=100.0,
                normalized_score=(b3 / N * 100) if N > 0 else 0,
                weight=20.0,
                raw_value=b3,
                raw_unit="keywords",
            ...
            AnalysisMetric(
                metric_name="Positions 21-100",
                category="ranking",
                metric_type="percentage",
                expected=100.0,
                normalized_score=((b4 + b5) / N * 100) if N > 0 else 0,
                weight=10.0,
                raw_value=b4 + b5,
                raw_unit="keywords",
            ...
            AnalysisMetric(
                metric_name="Not Found in Top 100",
                category="ranking",
                metric_type="percentage",
                expected=0.0,
                normalized_score=(b0 / N * 100) if N > 0 else 0,
                weight=10.0,
                raw_value=b0,
                raw_unit="keywords",
        ...
        return CategoryScore(
            ...
            confidence=confidence,
            confidence_factors=[
                f"{N} keywords tracked",
                f"Coverage: {coverage_rate:.0%}",
                f"Search engine: Google, Country: {country.upper()}, Device: {device}",
            ],
            limitations=[
    ...
    def _build_architecture_breakdown(
        ...
        metrics = [
            ...
            AnalysisMetric(
                metric_name="Orphan Pages",
                category="architecture",
                metric_type="percentage",
                expected=100.0,
                normalized_score=orphan_rate,
                weight=20.0,
                raw_value=orphan_count,
                raw_unit="pages",
            ...
            AnalysisMetric(
                metric_name="Internal Link Density",
                category="architecture",
                metric_type="range",
                expected=100.0,
                normalized_score=link_density_score,
                weight=20.0,
                raw_value=avg_links,
                raw_unit="links/page",
                ...
                status="PASS"
                if link_density_score >= 80
                else "NEEDS IMPROVEMENT"
                if link_density_score >= 40
                else "POOR",
            ...
            AnalysisMetric(
                metric_name="Internal Graph PageRank Equity Distribution",
                category="architecture",
                metric_type="range",
                expected=100.0,
                normalized_score=gini_score,
                weight=15.0,
                raw_value=gini,
                raw_unit="Gini coefficient",
                raw_evidence=f"Internal Graph PageRank Gini: {gini}"
                if gini is not None
                else "Insufficient data for Gini calculation",
                ...
                evidence=f"Internal Graph PageRank Gini coefficient: {gini}"
                if gini is not None
                else ("Insufficient pages with non-zero Internal Graph "
                      "PageRank for Gini calculation"),
                evidence_quality="CALCULATED" if gini is not None else "UNAVAILABLE",
                limitations="This is Internal Graph PageRank — NOT Google's PageRank. It measures"
                    "internal link equity distribution within the site only.",
                why_it_matters="Uneven internal PageRank distribution means some pages receive"
                    "disproportionate authority while others are starved.",
                remediation="Redistribute internal links to spread authority more evenly across"
                    "important pages.",
                sample_size=total,
                data_coverage=1.0 if gini is not None else 0.0,
                confidence="HIGH" if gini is not None else "UNABLE TO VERIFY",
            ),
            AnalysisMetric(
                metric_name="Max Crawl Depth",
                category="architecture",
                metric_type="range",
                expected=100.0,
                normalized_score=max_depth_score,
                weight=20.0,
                raw_value=max_d,
                raw_unit="clicks",
                ...
                status="PASS"
                if max_depth_score >= 80
                else "NEEDS IMPROVEMENT"
                if max_depth_score >= 40
                else "POOR",
        ...
        return CategoryScore(
            ...
            overall_score=round(overall, 1),
            score_status="ASSESSED",
            scoring_methodology="Score = sum(weight * normalized_score) / sum(weights) across 5"
                "metrics.",
            metrics=metrics,
            sample_size=total,
            data_coverage=1.0,
            ...
            limitations=[
                "Internal Graph PageRank is NOT Google's PageRank.",
                "Gini requires >=3 pages with non-zero PageRank.",
            ],
    ...
    def _build_aio_breakdown(
        ...
        metrics = [
            AnalysisMetric(
                metric_name="AIO Presence Rate",
                category="aio",
                metric_type="percentage",
                expected=100.0,
                normalized_score=presence_rate * 100,
                weight=30.0,
                raw_value=Q_aio,
                raw_unit="queries",
                ...
                evidence=f"AI Overviews detected for {Q_aio} of {Q} target keywords in"
                    f"{country.upper()} Google search",
                evidence_quality="OBSERVED",
                sample_size=Q,
                data_coverage=coverage,
            ...
            AnalysisMetric(
                metric_name="Target Citation Rate",
                category="aio",
                metric_type="percentage",
                expected=100.0,
                normalized_score=citation_rate * 100,
                weight=50.0,
                raw_value=Q_cited,
                raw_unit="queries",
            ...
            AnalysisMetric(
                metric_name="Competitor Non-Citation Rate",
                category="aio",
                metric_type="percentage",
                expected=100.0,
                normalized_score=(1 - comp_citation_rate) * 100,
                weight=20.0,
                raw_value=Q_comp_cited,
                raw_unit="queries",
                ...
                evidence=f"Competitors cited in {Q_comp_cited} of {Q_aio} AI Overview queries"
                    f"(higher = worse)",
                evidence_quality="OBSERVED",
                sample_size=Q_aio,
                data_coverage=coverage,
        ...
        return CategoryScore(
            ...
            overall_score=round(overall, 1),
            score_status="ASSESSED",
            scoring_methodology=f"AIO Score = (Presence * 30 + Citation * 50 +"
                f"CompetitorNonCitation * 20). Coverage: {coverage:.0%}.",
            metrics=metrics,
            sample_size=Q,
            data_coverage=coverage,
    ...
    def _build_geo_breakdown(
        ...
        metrics = [
            AnalysisMetric(
                metric_name="Target Mention Rate",
                category="geo",
                metric_type="percentage",
                expected=100.0,
                normalized_score=mention_rate * 100,
                weight=50.0,
                raw_value=Q_mentioned,
                raw_unit="queries",
                ...
                evidence=(
                    f"Target brand mentioned in {Q_mentioned} of "
                    f"{Q} generative engine queries"
                ),
                evidence_quality="OBSERVED",
                sample_size=Q,
                data_coverage=coverage,
            ...
            AnalysisMetric(
                metric_name="Competitor Mention Rate",
                category="geo",
                metric_type="percentage",
                expected=0.0,
                normalized_score=(1 - comp_mention_rate) * 100,
                weight=30.0,
                raw_value=Q_comp_mentioned,
                raw_unit="queries",
                raw_evidence=f"Competitors mentioned in {Q_comp_mentioned} of {Q} queries",
                normalization_method="(1 - competitor_mentioned / total_queries) * 100 (lower"
                    "competitor mentions = better)",
                status="PASS" if comp_mention_rate < 0.3 else "POOR",
                evidence=f"Competitors mentioned in {Q_comp_mentioned} of {Q} generative engine"
                    f"queries",
                evidence_quality="OBSERVED",
                sample_size=Q,
                data_coverage=coverage,
            ...
            AnalysisMetric(
                metric_name="Average Mentions per Query",
                category="geo",
                metric_type="range",
                expected=100.0,
                normalized_score=min(100, geo.avg_mention_count * 20),
                weight=20.0,
                raw_value=geo.avg_mention_count,
                raw_unit="mentions/query",
                raw_evidence=f"Average {geo.avg_mention_count:.1f} mentions per query where target"
                    f"mentioned",
                ...
                evidence_quality="CALCULATED",
                sample_size=Q_mentioned if Q_mentioned > 0 else 0,
                data_coverage=coverage,
                confidence="MEDIUM",
            ),
        ]
        ...
        return CategoryScore(
            ...
            overall_score=round(overall, 1),
            score_status="ASSESSED",
            scoring_methodology=f"GEO Score = (TargetMention * 50 + CompetitorNonMention * 30 +"
                f"AvgMentions * 20). Coverage: {coverage:.0%}. Engines: {engines_tested}.",
            metrics=metrics,
            sample_size=Q,
            data_coverage=coverage,
            confidence="HIGH" if coverage >= 0.8 else "MEDIUM" if coverage >= 0.5 else "LOW",
    ...
    def _analyze_onpage_seo(self, content_health: SiteContentHealth) -> list[SearchRecommendation]:
            ...
            recs.append(
                SearchRecommendation(
                    ...
                    description=(
                        ...
                        f"Meta descriptions are your SERP ad copy — they directly impact"
                            f"click-through rates. "
                        f"Write unique, compelling 150-160 character descriptions with target"
                            f"keywords."
                    ),
            ...
            recs.append(
                SearchRecommendation(
                    ...
                    description=(
                        ...
                        f"Search engines prefer comprehensive content. Expand with relevant"
                            f"details, "
        ...
        if hasattr(content_health, "top_issues"):
            missing_schema = sum(
                1 for issue, count in content_health.top_issues if "schema" in issue.lower()
            )
            if missing_schema > 0:
                recs.append(
                    SearchRecommendation(
        ...
        stale_content = sum(
            1 for issue, count in content_health.top_issues if "stale" in issue.lower()
        )
        if stale_content > 0:
            recs.append(
                SearchRecommendation(
                    ...
                    description=(
                        "Content freshness is a ranking factor. Update old content with current"
                            "data, "
        ...
        poor_readability = sum(
            1 for issue, count in content_health.top_issues if "readability" in issue.lower()
        )
        ...
        keyword_stuffing = sum(
            1 for issue, count in content_health.top_issues if "stuffing" in issue.lower()
        )
        ...
        low_internal = sum(
            1 for issue, count in content_health.top_issues if "internal" in issue.lower()
        )
        if low_internal > 0:
            recs.append(
                SearchRecommendation(
    ...
    def _build_unified_recommendations(
        ...
        *,
        ...
        if technical_health.warning_issues > 5:
            recs.append(
                SearchRecommendation(
                    ...
                    description="Multiple technical warnings that could impact crawlability and"
                        "rankings.",
        ...
        if content_health.thin_content_pages > 0:
            recs.append(
                SearchRecommendation(
                    ...
                    description="Pages with insufficient content unlikely to rank. Expand or"
                        "consolidate.",
        ...
        if content_health.duplicate_groups > 0:
            recs.append(
                SearchRecommendation(
                    ...
                    description="Near-duplicate pages compete with each other. Consolidate or"
                        "canonicalize.",
            ...
            if aio_analysis.ai_overviews_present > 0 and aio_analysis.target_cited_count == 0:
                recs.append(
                    SearchRecommendation(
                        category=RecommendationCategory.AIO,
                        priority=RecommendationPriority.CRITICAL,
                        title=f"AI Overviews present for {aio_analysis.ai_overviews_present}"
                            f"keywords but site not cited",
                        description=(
                            f"AI Overviews appear for {aio_analysis.ai_overviews_present} of your"
                                f"target keywords, "
                            f"but your site is not cited in any. Create authoritative, concise"
                                f"answers "
            ...
            if aio_analysis.target_citation_rate < 0.3 and aio_analysis.ai_overviews_present > 0:
                recs.append(
                    SearchRecommendation(
                        ...
                        description="Your site is rarely cited in AI Overviews despite their"
                            "presence. Optimize content structure.",
        ...
        if geo_analysis:
            if (
                geo_analysis.target_mentioned_count == 0
                and geo_analysis.competitor_mentioned_count > 0
            ):
                recs.append(
                    SearchRecommendation(
                        ...
                        description=(
                            f"Competitors mentioned {geo_analysis.competitor_mentioned_count} times"
                                f"across "
                            f"generative engines but your brand is absent. Build authority and"
                                f"citations."
                        ),
                        supporting_metrics={
            ...
            if geo_analysis.mention_rate < 0.2:
                recs.append(
                    SearchRecommendation(
                        ...
                        description="Generative engines rarely mention your brand. Focus on"
                            "authoritative content and citations.",
            ...
            for opp in geo_analysis.top_opportunities[:5]:
                recs.append(
                    SearchRecommendation(
                        ...
                        description=opp.get(
                            "action", "Build authority for generative engine mentions"
                        ),
                        affected_keywords=(opp["keyword"],),
                        supporting_metrics={
                            "competitors_mentioned": opp.get("competitors_mentioned", 0)
                        },
                        confidence=0.75,
                    )
                )
        ...
        if architecture_health.orphan_pages > 0:
            recs.append(
                SearchRecommendation(
                    ...
                    description="Pages with no internal links can't be discovered by users or"
                        "search engines.",
        ...
        if architecture_health.avg_depth > 4:
            recs.append(
                SearchRecommendation(
                    ...
                    priority=RecommendationPriority.MEDIUM,
                    title=(
                        f"Deep site architecture "
                        f"(avg depth: {architecture_health.avg_depth:.1f})"
                    ),
                    description="Important pages buried too deep. Flatten architecture for better"
                        "crawl efficiency.",
            ...
            if (
                ...
                recs.append(
                    SearchRecommendation(
                        ...
                        description=(
                            "Zero keywords ranking in top 10. Fundamental content/authority issues."
                        ),
                        confidence=0.95,
                    )
                )
