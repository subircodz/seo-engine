...
from sie.config import CrawlerSettings, MigrationSettings, SearchProviderSettings, Settings
...
def test_migration_settings_loads_database_url_without_runtime_validation(tmp_path) -> None:
    env_file = tmp_path / ".env"
    env_file.write_text(
        "SIE_ENVIRONMENT=production\n"
        "SIE_DEBUG=true\n"
        "SIE_DATABASE__AUTO_MIGRATE=true\n"
        "SIE_DATABASE_URL=sqlite+aiosqlite:///./from-migration-env.db\n",
        encoding="utf-8",
    )

    settings = MigrationSettings(_env_file=env_file)

    assert settings.database_url == "sqlite+aiosqlite:///./from-migration-env.db"
