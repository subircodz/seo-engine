...
class TestPDFTemplateRendering:
    def test_accessible_report_renders(self):
        ...
        result = SiteAnalysisResult(
            ...
            technical=SiteTechnicalHealth(
                pages_crawled=5, critical_issues=0, warning_issues=2, info_issues=0,
                indexable_pages=5, non_indexable_pages=0, avg_load_time_ms=500.0,
                core_web_vitals_pass=True, has_ssl=True, robots_txt_valid=True,
                sitemap_exists=True,
                metric_details=[
                    TechnicalMetricDetail(
                        "SSL/HTTPS", 10, 100, 100, "PASS",
                        evidence="OK",
                    ),
                    TechnicalMetricDetail(
                        "Crawlability", 15, 100, 95, "PASS",
                        evidence="95% indexable",
                    ),
                    TechnicalMetricDetail(
                        "Performance", 20, 100, 80, "PASS",
                        evidence="500ms avg",
                    ),
                    TechnicalMetricDetail(
                        "Robots.txt", 10, 100, 75,
                        "NEEDS IMPROVEMENT",
                        evidence="Missing directives",
                        remediation="Update robots.txt",
                    ),
                    TechnicalMetricDetail(
                        "XML Sitemap", 10, 100, 80,
                        "NEEDS IMPROVEMENT",
                        evidence="Incomplete",
                        remediation="Add all pages",
                    ),
                    TechnicalMetricDetail(
                        "Issue Density", 15, 100, 95, "PASS",
                        evidence="Low density",
                    ),
                    TechnicalMetricDetail(
                        "Indexability", 20, 100, 100, "PASS",
                        evidence="All indexable",
                    ),
                ],
            ),
            content=SiteContentHealth(
                pages_analyzed=5, avg_quality_score=0.7,
                thin_content_pages=0, duplicate_groups=0,
                missing_h1_pages=0, missing_meta_desc_pages=0,
                low_word_count_pages=0,
            ),
            architecture=SiteArchitectureHealth(
                total_pages=5, total_internal_links=20,
                avg_depth=2.0, orphan_pages=0, max_depth=4,
            ),
            access_status=AccessStatus(
            ...
            website_type=WebsiteTypeInfo(
                technology="WordPress", confidence=0.8,
    ...
    def test_blocked_report_renders(self):
        ...
        result = SiteAnalysisResult(
            ...
            overall_score=0.0,
            access_status=AccessStatus(
                accessible=False, status_code=403, analysis_status="BLOCKED",
                protection_detected="Cloudflare",
                protection_details="JavaScript challenge",
                bypass_attempted=True, bypass_succeeded=False,
                bypass_methods_used=["User-agent rotation"],
                block_reason="Cloudflare protection",
                analyzed_page_is_challenge=True,
            ),
            website_type=WebsiteTypeInfo(
                technology="Unknown", confidence=0.0,
